...
public class SwiftAudioWaveformsPlugin: NSObject, FlutterPlugin {
    ...
    init(
        registrar: FlutterPluginRegistrar, flutterChannel: FlutterMethodChannel
    ) {
    ...
    public func handle(
        ...
        case Constants.startRecording:
            let qualityRawValue = args?[Constants.quality] as? Int
            audioRecorder.startRecording(
                result,
                args?[Constants.path] as? String,
                args?[Constants.encoder] as? Int,
                args?[Constants.sampleRate] as? Int,
                args?[Constants.bitRate] as? Int,
                Constants.fileNameFormat,
                args?[Constants.useLegacyNormalization] as? Bool,
                overrideAudioSession: (args?[Constants.overrideAudioSession]
                    as? Bool) ?? true,
                appendPath: (args?[Constants.appendPath]
                    as? Bool) ?? false,
                stopAnyRecording: (args?[Constants.stopAnyRecording]
                    as? Bool) ?? false,
                quality: AVAudioQuality(
                    rawValue: qualityRawValue ?? AVAudioQuality.high.rawValue)!
            )
            break
        ...
        case Constants.preparePlayer:
            ...
            if key != nil {
                ...
                audioPlayers[key!]?.preparePlayer(
                    path: args?[Constants.path] as? String,
                ...
                result(
                    FlutterError(
                        ...
                        message: "Can not prepare player",
                        details: "Player key is null"))
            }
            break
        ...
        case Constants.finishMode:
            ...
            if key != nil {
                audioPlayers[key!]?.setFinishMode(
                    result: result, releaseType: releaseType)
            } else {
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Can not set release mode",
                        details: "Player key is null"))
            }
        ...
        case Constants.seekTo:
            let key = args?[Constants.playerKey] as? String
            if key != nil {
                audioPlayers[key!]?.seekTo(
                    args?[Constants.progress] as? Int, result)
            } else {
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Can not seek to postion",
                        details: "Player key is null"))
            }
        case Constants.setVolume:
            let key = args?[Constants.playerKey] as? String
            if key != nil {
                audioPlayers[key!]?.setVolume(
                    args?[Constants.volume] as? Double, result)
            } else {
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Can not set volume",
                        details: "Player key is null"))
            }
        case Constants.setRate:
            let key = args?[Constants.playerKey] as? String
            if key != nil {
                audioPlayers[key!]?.setRate(
                    args?[Constants.rate] as? Double, result)
            } else {
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Can not set rate",
                        details: "Player key is null"))
            }
        case Constants.getDuration:
            ...
            if key != nil {
                do {
                    if type == 0 {
                        try audioPlayers[key!]?.getDuration(
                            DurationType.Current, result)
                    } else {
                        try audioPlayers[key!]?.getDuration(
                            DurationType.Max, result)
                    }
                } catch {
                    result(
                        FlutterError(
                            code: "", message: "Failed to get duration",
                            details: nil))
                }
            } else {
                result(
        ...
        case Constants.extractWaveformData:
            ...
            if key != nil {
                createOrUpdateExtractor(
                    playerKey: key!, result: result, path: path,
    ...
    func initPlayer(playerKey: String) {
        if audioPlayers[playerKey] == nil {
            let newPlayer = AudioPlayer(
                plugin: self, playerKey: playerKey, channel: flutterChannel)
            audioPlayers[playerKey] = newPlayer
        }
    }

    func createOrUpdateExtractor(
        playerKey: String, result: @escaping FlutterResult, path: String?,
        noOfSamples: Int?
    ) {
        if !(path ?? "").isEmpty {
            do {
                ...
                if audioUrl == nil {
                    result(
                        FlutterError(
                            ...
                            details:
                                "If path contains `file://` try removing it"))
                    return
                }
                let newExtractor = try WaveformExtractor(
                    url: audioUrl!, flutterResult: result,
                    channel: flutterChannel)
                extractors[playerKey] = newExtractor
                let data = newExtractor.extractWaveform(
                    samplesPerPixel: noOfSamples, playerKey: playerKey)
                newExtractor.cancel()
                ...
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Failed to decode audio file", details: nil))
            }
        } else {
            result(
                FlutterError(
