...
public class AudioRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    public func startRecording(
        _ result: @escaping FlutterResult, _ path: String?, _ encoder: Int?,
        _ sampleRate: Int?, _ bitRate: Int?, _ fileNameFormat: String,
        _ useLegacy: Bool?, overrideAudioSession: Bool, appendPath: Bool,
        stopAnyRecording: Bool,
        quality: AVAudioQuality
    ) {
        useLegacyNormalization = useLegacy ?? false
        
        print("appendPath: \(appendPath)")
        print("stopAnyRecording: \(stopAnyRecording)")
        print("quality: \(quality.rawValue)")
        print("quality (name): \(String(describing: quality))")

        if stopAnyRecording == true {
            self.audioUrl = nil
        ...
        if path == nil {
            ...
            let documentDirectory = getDocumentDirectory(result)
            self.path = "\(documentDirectory)\(path!)"
        } else {
            self.path = path
        }
            ...
            if audioUrl == nil {
                result(
                    FlutterError(
                        ...
                        message: "Failed to initialise file URL", details: nil))
            }
            audioRecorder = try AVAudioRecorder(
                url: audioUrl!,
                settings: bitRate != nil
                    ? settingsWithBitrate as [String: Any]
        ...
        -> String
    {
        ...
        if directory.isEmpty {
            ...
            result(
                FlutterError(
                    code: Constants.audioWaveforms,
                    message: "The document directory does't exists",
                    details: nil))
        }
        return directory
    }
